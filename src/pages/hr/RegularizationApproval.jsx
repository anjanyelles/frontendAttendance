...
const RegularizationApproval = () => {
  ...
  const loadRegularizationRequests = async () => {
    try {
      ...
      // Filter only manager approved requests
      const regs = response.data.regularizationRequests || response.data || []
      const managerApproved = regs.filter(
        (reg) => reg.status === REGULARIZATION_STATUS.MANAGER_APPROVED
      )
      setRegularizations(managerApproved)
    } catch (error) {
  ...
  const regularizationColumns = [
    ...
    },
    {
      ...
      render: (row) => <span className="truncate max-w-xs">{row.managerComments || '-'}</span>,
    },
    {
      header: 'Status',
      accessor: 'status',
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="Manager Approved Regularization Requests">
        <Table
          columns={regularizationColumns}
          data={regularizations}
          loading={loading}
          emptyMessage="No regularization requests pending HR approval"
        />
      </Card>
